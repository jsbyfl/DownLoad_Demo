...
//获取某用户名的附件文件夹
+ (NSString *)getUserDocument{
    NSString *userName = [[self class] getUserName];
    return [[self class] filePath:userName];
}
...
+(NSString *)getTargetFilePath:(NSString *)fileName{
    ...
    //文件夹目录
    NSString *document = [[self class] getUserDocument];
    //设置文件存储路径
    ...
    if ([fileManager fileExistsAtPath:filePath]) {
        ...
        //DLog(@"该文件还未下载");
        //该文件还未下载,再判断该用户名的所在文件夹是否存在，如果存在，则设置文件存储路径，如果未创建，则先创建该账号的文件夹
        //=================================================
        if ([fileManager isReadableFileAtPath:document]) {
            //DLog(@"文件夹已存在");
        }else{
            //DLog(@"文件夹还未创建");
...
+ (NSString *)getFinishedPath{
    NSString *userName = [[self class] getUserName];
    NSString *finishPath = [NSString stringWithFormat:@"%@_finishPlist.plist",userName];
    return [[self class] filePath:finishPath];
}
...
//            finishedFile.fileSize=[FileHelper getFileSizeString:[NSString stringWithFormat:@"%ld",(long)length]];
...
+ (NSArray *)filesSupports{
    NSArray *arrays = [NSArray arrayWithObjects:@"doc",@"docx",@"jpg",@"jpeg",@"bmp",@"pdf",@"png",@"ppt",@"pptx",@"txt",@"xls",@"xlsx",@"gif",
                                                @"DOC",@"DOCX",@"JPG",@"JPEG",@"BMP",@"PDF",@"PNG",@"PPT",@"PPTX",@"TXT",@"XLS",@"XLSX",@"GIF",nil];
    return arrays;
}
